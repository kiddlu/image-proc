    ...
    for (uint32_t x = 0; x < frame->width; x++) {
        for (uint32_t y = 0; y< frame->height; y++) {
            if (is_valid_point(frame, x, y)) {
                ...
                if ( dvalue < 1.0 )
                    bpclass[0]++;
                else if( dvalue >= 1.0 && dvalue < 5.0)
                    bpclass[1]++;
                else if( dvalue >= 5.0 && dvalue < 10.0)
                    bpclass[2]++;
                else if( dvalue >= 10.0 && dvalue < 15.0)
                    bpclass[3]++;
                else if( dvalue >= 15.0 && dvalue < 25.0)
                    bpclass[4]++;
                else if( dvalue >= 25.0)
                    bpclass[5]++;
            }
...
void fill_cut_frame(struct imiFrame *cut, struct imiFrame *orig, int xofs, int yofs)
{
    int x,y;
    unsigned short *p = cut->data;
    for(y=0; y < orig->height; y++)
        for(x=0; x < orig->width; x++) {
            if((x >= xofs) && (x < xofs + cut->width) &&
               (y >= yofs) && (y < yofs + cut->height)) {
               *p = get_depth_data(orig, x, y);
               p++;
            }
        }
}
...
int main(int argc, void **argv)
{
    ...
    cut_frame.width = get_ini_key_int(config_fpath, "Settings", "CutWidth");
    cut_frame.height = get_ini_key_int(config_fpath, "Settings", "CutHeight");
    cut_frame.total_pixels = cut_frame.width * cut_frame.height;
    cut_frame.size = sizeof(unsigned short) * cut_frame.total_pixels;
    cut_frame.data = (unsigned short *)malloc(cut_frame.size);

    int cut_x_offset = get_ini_key_int(config_fpath, "Settings", "CutXOffset");
    int cut_y_offset = get_ini_key_int(config_fpath, "Settings", "CutYOffset");
    float bp_threadhold = (float)get_ini_key_int(config_fpath, "Settings", "LongThreadhold");
    ...
    for (; frame_num < frame_total; frame_num++) {
        ...
        fill_cut_frame(&cut_frame, &frame, cut_x_offset, cut_y_offset);
        
        get_valid_pixels(&cut_frame);
        cv_fit_plane(&cut_frame);

        /*calc value*/
        float interity = calc_plane_integrity(&cut_frame);
        float avgdepth = calc_avg_depth(&cut_frame);
        float bp_rate = calc_badpoint_rate(&cut_frame, bp_threadhold);
        ...
        clac_badpoint_class(&cut_frame, bpclass);
